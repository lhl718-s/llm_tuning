...
from trl import GRPOTrainer, SFTTrainer, SFTConfig
...
class RLpipeline:
    def __init__(
        ...
        self.config.output_dir = os.path.join(OUTPUT_BASE_DIR, self.output_dir)
    ...
    def load_model_and_tokenizer(self):
        ...
        if not os.path.exists(self.model_path):
            ...
            if self.training_config_params['training_launch_type'] == 'unsloth':
                self.model, self.tokenizer = FastLanguageModel.from_pretrained(model_name=self.model_path,
                                                                               max_seq_length=1024, dtype=torch.float16,
                                                                               load_in_4bit=True,
    ...
    def setup_trainer(self):
        ...
        if self.training_config_params['train_mode'] == "finetune_full":
            ...
            self.trainer = SFTTrainer(
                ...
                train_dataset=self.dataset['train'].train_test_split(test_size=0.2),
                processing_class=self.tokenizer,
            )
        elif self.training_config_params['train_mode'] == 'reinforce learning(without code start)':
            if self.training_config_params['rl_mode'] == 'lora':
                #  self.model = get_peft_model(self.model, self.rl_lora_config)
...
class TestChatModel:
    ...
    def generate_response(self, model, tokenizer, user_question, device, max_new_tokens=1024, temperature=0.7,
                          top_p=0.9):
        messages = [
...
if __name__ == "__main__":
    ...
    print(f'rl_pipeline {os.getcwd()}', flush=True)
    # raise 1
    # 1. 传入参数
    ...
    train_mode = "finetue_lora"
    # 第一位参数：训练模式 sft 微调.  rl_with_code 强化学习.  rl 强化学习（仅强化）
    rl_mode = None
    if args[0] == "sft":
        train_mode = 'finetue_lora'
    ...
    # 第二位参数：强化方向 format 格式强化.   deep_thinking 思考深度强化
    if args[1] == "format":
        rl_direction = "format"
    elif args[1] == "deep_thinking":
        rl_direction = "deep thinking"
    else:
        rl_direction = None

    # 第三位参数：训练模式 speed 速度优先.   stablity 稳定优先
    if args[2] == "speed":
        train_preference = "speed"
    elif args[2] == "stablity":
        train_preference = "stablity"
    else:
    ...
    # training_launch_type = 'unsloth'
    # train_preference =  None
    output_dir = '/root/data/GRPO7B/outputs'
    ...
    # 指定 模型路径 和 数据集路径
    model_path = "/root/.cache/modelscope/hub/deepseek-ai/DeepSeek-R1-Distill-Qwen-7B"
    # data_file_path = "/root/liancairltraning/pipeline/datasets/q_a_jituanzhishi2.0_modified.json"
    data_file_path = os.path.join(os.getcwd(), "datasets/data.json")
