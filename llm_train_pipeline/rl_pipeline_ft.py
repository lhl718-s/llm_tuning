...
class RLpipeline:
    ...
    def setup_trainer(self):
        ...
        if self.training_config_params['train_mode'] == "finetune_full":
            ...
            from transformers import Trainer
            if self.training_config_params['training_launch_type'] == 'unsloth':
                from unsloth import TrainerForNativeFinetuning
                self.trainer = TrainerForNativeFinetuning(
                    model=self.model,
                    args=self.rl_config,
                    train_dataset=self.dataset_lora,
                    data_collator=data_collator
            ...
            # Commented out GRPOTrainer due to compatibility issues
